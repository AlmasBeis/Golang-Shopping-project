...
func (c *Controller) InitRoutes() *mux.Router {
	...
	auth.HandleFunc("/sign-in", c.login).Methods("POST")

	auth.HandleFunc("/sign-up", c.register).Methods("POST")
	...
	comments.Use(c.userIdentity)
	category.Use(c.userIdentity)
	purchase.Use(c.userIdentity)
	users.Use(c.userIdentity)
	...
	items.HandleFunc("/setRating/{id}", c.setItemRating).Methods("PUT")
